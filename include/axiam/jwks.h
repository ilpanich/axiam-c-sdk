 ...
 * 300 seconds.
 *
 * Verification is STRICT by default (SEC-071): on top of the signature,
 * axiam_jwt_verify() enforces the `exp`/`nbf` lifetime and binds the token's
 * `tenant_id` claim to the client's configured tenant. The JWKS endpoint is
 * organization-wide, so a valid signature alone does NOT imply the token was
 * minted for this client's tenant. axiam_jwt_verify_ex() exposes the policy
 * flags for callers that deliberately want a weaker check.
 */
#ifndef AXIAM_JWKS_H
#define AXIAM_JWKS_H
...
/** The default policy: signature + lifetime + tenant binding. */
#define AXIAM_JWT_VERIFY_STRICT (AXIAM_JWT_VERIFY_EXPIRY | AXIAM_JWT_VERIFY_TENANT)
 ...
 * present, must not be in the future; and `tenant_id` must be present and
 * equal to the client's configured tenant. Tenant binding requires the client
 * to know its tenant UUID — configure `tenant_id` (or complete a login, which
 * resolves it from the session claims); a slug-only client cannot bind and
 * fails closed.
 *
 ...
 * AXIAM_ERR_NETWORK (JWKS could not be fetched).
 */
