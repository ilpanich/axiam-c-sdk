...
| C | implemented — REST surface, per its `CLAUDE.md` carve-out (contract 1.48). All three §28.1 operations, plus `axiam_protected_resource_metadata_url` (§28.1's `metadata_url` MUST, not itself named in §28.7's C row) and the guard-side `axiam_require_auth_mcp`/`axiam_require_access_mcp` (additive over `axiam_require_auth`/`axiam_require_access`, mirroring `axiam_require_access_uma`'s precedent). No `serve_` function (§28.3/§28.7 — no router); the CivetWeb adapter is documented. |
| Rust, Python, Java, Kotlin, C#, PHP, Go, Swift, C++ | not yet |
