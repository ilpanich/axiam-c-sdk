...
static void test_oauth2_clients_create_registration_token_survives_oom(void) {
    for (long n = 1; n <= ALLOC_DEPTH; n++) {
        mgmt_reset();
        ...
        arm(n);
        (void) axiam_oauth2_clients_create_registration_token(c, &body, &result, &err);
        disarm();
...
static void test_oauth2_clients_list_registration_tokens_survives_oom(void) {
    for (long n = 1; n <= ALLOC_DEPTH; n++) {
        mgmt_reset();
        ...
        arm(n);
        (void) axiam_oauth2_clients_list_registration_tokens(c, &result, &err);
        disarm();
