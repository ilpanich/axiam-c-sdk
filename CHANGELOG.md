...
- **A malformed `SubjectAltName` in `axiam_certificates_generate()` /
  `axiam_certificates_sign_csr()` is now refused client-side, before any
  request** (CONTRACT.md §27.13 / §27.4 rule 2, contract 1.52 N-3, C-12).
  `SubjectAltName` is externally tagged (`{"dns": …}` XOR `{"ip": …}`), but
  this SDK's `axiam_mgmt_subject_alt_name_t` represents it as a `kind` enum
  plus one `value` string — a shape that CAN hold a value neither wire form
  expresses: `value == NULL`, or a `kind` outside the two the spec defines.
  Previously, an element with `value == NULL` was silently DROPPED from the
  `subject_alt_names` array (`_build()` returned `NULL` for it and the
  caller's `if (sub) cJSON_AddItemToArray(...)` skipped it), and an element
  with an out-of-range `kind` was silently MIS-TAGGED onto `"dns"`
  (`_build()`'s `switch` `default` arm) — both sent a request that did not
  say what the caller asked for. Both operations now validate every element
  of `subject_alt_names` with the new `axiam_mgmt_subject_alt_name_valid()`
  before building the path or the body, and refuse with `AXIAM_ERR_NETWORK`
  ("SDK programming error", §27.4 rule 2) and zero wire calls if any is
  invalid. Generated by `scripts/gen_management.py` (`_valid()` alongside
  each externally-tagged type's `_parse()`/`_build()`, and a validation loop
  emitted for any operation whose body carries an array of one), not
  hand-patched into `management_ops.c`/`management_models.c`, so the drift
  check (`python3 scripts/gen_management.py --check`) stays green.

  **Migration:** a caller that was (unknowingly) sending a `SubjectAltName`
...
- **CONTRACT.md §10.4 — an optional session-revocation feed poller (contract
  ...
  `axiam_revocation_entry_for()` and the three bounds the contract names, in a
  new `axiam/revocation.h`.
  ...
  Nothing changes unless you enable it: a client that does not is byte-for-byte
  the client it was, and never fetches the feed at all. It never fetches on the
  request path after the first call, and a feed that is down is not retried on
  every request (the interval runs from the last *attempt*). And it never fails
  closed: an unreachable feed, a non-`200`, an unparseable body or an unknown
  `alg` all behave exactly as no feed at all, and specifically not as an empty
  list, which would assert that nothing has been revoked. A token with no `sid`
  is never matched against it, and there is no fallback to `jti`.
...
- Two tests pinning CONTRACT.md §16 against the server's new answer for a
  ...
  changed: §16.3 already retried `5xx` on an eligible operation and
  `axiam_retry_delay_ms` already honoured `Retry-After` as a floor. Both halves
  are asserted through the public surface with a wire count, because a retry
  policy nobody exercises that way is the failure §16.7 exists for.
...
- **A malformed `mtls_endpoint_aliases` entry is now refused instead of falling
  ...
  The refusal is `AXIAM_ERR_AUTH`, not `AXIAM_ERR_NETWORK`: nothing failed in
  transport, and §16.3 retries `AXIAM_ERR_NETWORK` and only that, so the other
  choice would have retried a permanent misconfiguration three times and
  reported it as a transient one. It is per endpoint, so one malformed alias
  leaves every other endpoint working, and a client with no certificate never
  reads the member at all.

  `oidc_preferred_endpoint()` is module-private and now returns an
  `axiam_error_kind_t` with the endpoint as an out-parameter; no public
  signature changed.
...
- Carry the tenant into the §26.2 redirect, and accept a caller-supplied dpop_jkt

- Model the two new RFC 8414 discovery members
...
- **RFC 8705 §5 `mtls_endpoint_aliases` (SDK contract 1.40, CONTRACT.md §21.3
  ...
  `axiam_mtls_endpoint_aliases_t`) plus a `has_mtls_endpoint_aliases` flag, and
  the §12 operations now prefer an alias over the top-level entry of the same
  name on any call made over mutual TLS — that is, from a client configured
  with `axiam_client_config_set_client_cert`. Five operations reach the token
  endpoint (`axiam_oidc_exchange`, `axiam_oidc_refresh`,
  `axiam_login_client_credentials`, `axiam_device_poll`,
  `axiam_token_exchange`), plus `axiam_introspect`, `axiam_revoke`,
  `axiam_device_authorize` and `axiam_oidc_par`.

  A clear flag means "this deployment terminates mutual TLS on the issuer's own
  host", never "mTLS is unsupported": a client without it keeps using the
  conventional endpoints instead of failing. Every member of the alias struct
  may be `NULL`, so an endpoint a partial object does not name falls back
  rather than failing the whole document. No alias is synthesised for
  `authorization_endpoint`, `end_session_endpoint` or `jwks_uri`, which are
  front-channel or public. `issuer` does not move, and §12.4 rule 3 still
  compares a token's `iss` against it by exact string — including for a token
  minted at an alias endpoint.

  The new members are appended to `axiam_oidc_config_t`, so existing source
  keeps compiling; a consumer linking against a prebuilt library must rebuild,
  as for any struct-layout change.

### Changed

- Record the 1.40 → 1.42 re-sync in CHANGELOG and README

- AXIAM no longer mints tenant_id, org_id or email into the ID token

- Re-sync the contract at 1.42 and regenerate the §27 surface
...
- Replace, never append, the tenant on a discovered endpoint
